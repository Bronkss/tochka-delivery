...
import { answerCallbackQuery, buildOrderKeyboard, editTelegramMessage, formatOrderMessage, getTelegramUserName, sendTelegramMessage, } from './_telegram.js';
...
async function answerAndFinish(res, callbackQueryId, text, showAlert = false) {
...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (update.message?.text?.startsWith('/chatid')) {
            await sendTelegramMessage(update.message.chat.id, `chat_id этой группы: <code>${update.message.chat.id}</code>`);
            return res.status(200).json({
                ok: true,
            });
        }
        ...
        if (!match) {
            return answerAndFinish(res, callback.id, 'Не понял команду', true);
        }
        ...
        try {
            ...
            if (!currentOrder) {
                ...
                return answerAndFinish(res, callback.id, 'Заказ не найден', true);
            }
            if (currentOrder.status === 'completed' ||
                currentOrder.status === 'cancelled') {
                await client.query('ROLLBACK');
                return answerAndFinish(res, callback.id, 'Этот заказ уже закрыт', true);
            }
            const courierTelegramId = String(callback.from.id);
            const courierName = getTelegramUserName(callback.from);
            if (currentOrder.courier_telegram_id &&
                currentOrder.courier_telegram_id !== courierTelegramId) {
                await client.query('ROLLBACK');
                return answerAndFinish(res, callback.id, `Заказ уже закреплён за ${currentOrder.courier_name}`, true);
            }
            if (action !== 'accept' && currentOrder.status === 'new') {
                await client.query('ROLLBACK');
                return answerAndFinish(res, callback.id, 'Сначала нужно принять заказ', true);
            }
            const shouldAssignCourier = action === 'accept' ||
        ...
        const chatId = updatedOrder.telegram_chat_id ??
            callback.message?.chat.id ??
            process.env.TELEGRAM_COURIER_CHAT_ID;
        const messageId = updatedOrder.telegram_message_id ??
            callback.message?.message_id;
        if (chatId && messageId) {
            try {
                await editTelegramMessage(chatId, Number(messageId), formatOrderMessage({
                    ...updatedOrder,
                    items: orderItems,
        ...
        return answerAndFinish(res, callback.id, getActionSuccessText(action));
    }
    catch (error) {
